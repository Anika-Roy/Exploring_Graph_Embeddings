...
def main():
    ...
    logging.basicConfig(format="%(levelname)s - %(asctime)s: %(message)s", level=logging.INFO,filename='flickr.log',filemode='w')

    data_np = load_mat('/home2/anika.roy/SMAI-project/datasets/flickr.mat')
    ...
    dw = DeepWalk(adjmat, 10, 128, 80, 40,1,1)

    dw.train(20, 0.05)

    # output embeddings to an output file
    dw.model.wv.save_word2vec_format('flickr_embeddings.txt')
