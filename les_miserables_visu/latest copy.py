...
class Node2Vec():
        ...
        def second_order_biased_random_walk(
                ...
                for i in range(2, walk_len):
                        current_node = walk[i - 1]
                        prev_node = walk[i - 2]

                        # Get neighbors and weights of the current node
                        neighbors, weights = adj_list[current_node][0], adj_list[current_node][1]

                        # Calculate transition probabilities
                        probabilities = [
                                1.0 / in_out_parameter if node == prev_node else
                                weights[self.binary_search(neighbors, node)] if self.binary_search(neighbors, node) != -1 else
                                1.0 / return_parameter
                        ...
                        new_node = np.random.choice(neighbors, p=probabilities)

                        # Append the new node to the walk
                        walk.append(new_node)
        ...
        def generate_n_walks_parallel(self, num_iters: int, num_cores):
                ...
                with concurrent.futures.ProcessPoolExecutor(max_workers=num_cores-10) as executor:
...
def sparse_matrix_to_adjacency_list(graph):
        ...
        for node in graph.nodes:
                neighbors = np.array(list(graph.neighbors(node)), dtype=int)  # Use int type for integers
                ...
                neighbors = neighbors[sort_order].astype(int)
                weights = weights[sort_order].astype(float)
...
def main():
        ...
        dw = Node2Vec(adjacency_list, 3, 16, 60, 3, 1, 0.5)
