...
def main():
    ...
    logging.basicConfig(format="%(levelname)s - %(asctime)s: %(message)s", level=logging.INFO,filename='cora.log',filemode='w')
...
# #     print(adjacency_list[100][0], adjacency_list[100][1])
    ...
    for node in Gnx.nodes:
        neighbors = np.array(list(Gnx.neighbors(node)), dtype=int)
        weights = np.array([float(Gnx[node][neighbor]['weight']) for neighbor in neighbors], dtype=float)
...
#         node_data["subject"] = node_data["subject"].astype('category')
    ...
    dw.model.wv.save_word2vec_format('cora.txt')
